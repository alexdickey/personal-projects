...
class Pinky(GameCharacter):
    ...
    def update(self):
        ...
        # Pinky decides at each intersection whether to go left, right
        ...
        if (on_vert and on_horz):
            if abs(up_down_part) > abs(left_right_part):
                ...
                if (abs(up_down_part)/up_down_part) == -1:
                    self.y_add = -(self.velocity)
            elif abs(left_right_part) > abs(up_down_part):
                self.x_add = self.velocity
                self.y_add = 0
                if (abs(left_right_part)/left_right_part) == -1:
                    self.x_add = -(self.velocity)
